    ...
    <div className="space-y-2 pb-2">
      ...
      {layer.type === 'text' && (
        <Section title="TEXT" open={open.content} onToggle={() => setOpen((x) => ({ ...x, content: !x.content }))}>
          ...
          <Field label="Accent Color"><div className="grid grid-cols-[36px_1fr] gap-2"><input type="color" className="h-8 w-9 rounded-md" value={rgbaToHex(layer.accentColor || '#00E676')} onChange={(e) => onUpdate({ accentColor: e.target.value })} /><input className="ui-input h-8 px-2" value={layer.accentColor || '#00E676'} onChange={(e) => onUpdate({ accentColor: e.target.value })} /></div></Field>
      ...
      {(layer.type === 'image' || layer.type === 'logo') && (
        <Section title="IMAGE" open={open.content} onToggle={() => setOpen((x) => ({ ...x, content: !x.content }))}>
          ...
          <Field label="Shape"><select className="ui-input h-8 w-full px-2" value={layer.shape || 'rect'} onChange={(e) => onUpdate({ shape: e.target.value })}><option value="rect">Rectangle</option><option value="circle">Circle</option></select></Field>
          <Field label="Border"><div className="grid grid-cols-[60px_36px] gap-2"><input type="number" className="num-input w-full" placeholder="Width" value={layer.borderWidth || 0} onChange={(e) => onUpdate({ borderWidth: Number(e.target.value) })} /><input type="color" className="h-8 w-9 rounded-md" value={rgbaToHex(layer.borderColor || '#000000')} onChange={(e) => onUpdate({ borderColor: e.target.value })} /></div></Field>
        </Section>
      )}
